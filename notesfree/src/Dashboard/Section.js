...
const Section = (props) => {
  ...
  useEffect(async () => {
    const result = await axios.get(
      'http://localhost:5000/notedElements/' + props.section_id
    )
    setSectionElements(result.data)
  }, [addNewElem]) // get all elements saved in section
  ...
  function newEelementAdded() {
    // close 'add new element' window
    setAddNewElem(false)
  }

  return ( // what's in current section
    <div className='currentSection__container' ref={openSectionContainer}>
      <div className='currentSection__navbar'>
        ...
        <button
          className='currentSection__add_btn'
          onClick={() => setAddNewElem(true)}
        >
          Dodaj nowy element do <span>{props.section_name}</span>
        </button>
        <button
          className='currentSection__exit_btn'
          onClick={() => setSectionClosed(true)}
        >
          <AiFillCloseSquare />
        </button>
      </div>
        ...
        {SectionElements.map((element) => {
          ...
          return (
            <AddedElement
              ...
              is_text_note_link={is_text_note_link}
              photo_url={photo_url}
